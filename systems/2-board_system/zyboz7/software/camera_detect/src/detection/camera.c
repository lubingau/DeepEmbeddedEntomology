...
{	
	...
	*/
	xil_printf("[INFO] Deleting file: %s\n\r", file_name);
	bool error = false;
	// SD card setup
	FRESULT res;
	FILINFO fno;
...
//	xil_printf("[INFO] SD card mounted\n\r");
	...
	if (res != FR_OK) {
		xil_printf(ANSI_COLOR_RED "[ERROR] Error deleting file: %d\n\r" ANSI_COLOR_RESET, res);
		error = true;
		goto ERROR;
	}

	xil_printf("[SUCCESS] File deleted: %s\n\r", file_name);

ERROR:
	if (error) {xil_printf(ANSI_COLOR_RED "[ERROR] Failed to delete the file\n\r" ANSI_COLOR_RESET);}
