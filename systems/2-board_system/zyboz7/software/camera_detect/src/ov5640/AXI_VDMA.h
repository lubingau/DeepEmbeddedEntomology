...
public:
	...
	AXI_VDMA(uint16_t dev_id, uint32_t frame_buf_base_addr, IrptCtl& irpt_ctl, uint16_t wr_irpt_id) :
...
//		while (XAxiVdma_ChannelIsRunning(&drv_inst_.WriteChannel)) ;
	...
	void startWrite()
	{
		XAxiVdma_DmaStart(&drv_inst_, XAXIVDMA_WRITE);
	}
	void stopRead()
	{
	...
	void startRead()
	{
		XAxiVdma_DmaStart(&drv_inst_, XAXIVDMA_READ);
	}
	int getFrameStoreAddr()
	{	// Get the current frame buffer index
		return XAxiVdma_CurrFrameStore(&drv_inst_, XAXIVDMA_READ);
	}
	uint32_t getFrameAddress(int idx)
	{	// Get the address of the idx-th frame buffer
		return context_.ReadCfg.FrameStoreStartAddr[idx];
	}
	int getFrmStore()
	{
		uint8_t FrmStoreNum;
		XAxiVdma_GetFrmStore(&drv_inst_, &FrmStoreNum, XAXIVDMA_WRITE);
		return FrmStoreNum;
	}
	int startReadParking(int idx)
	{	// Force the read frame buffer to be the idx-th one
		return XAxiVdma_StartParking(&drv_inst_, idx, XAXIVDMA_READ);
	}
	void stopReadParking()
	{	// Stop read parking
		XAxiVdma_StopParking(&drv_inst_, XAXIVDMA_READ);
	}
	int startWriteParking(int idx)
	{	// Force the write frame buffer to be the idx-th one
		return XAxiVdma_StartParking(&drv_inst_, idx, XAXIVDMA_WRITE);
	}
	void stopWriteParking()
	{	// Stop write parking
		XAxiVdma_StopParking(&drv_inst_, XAXIVDMA_WRITE);
	}
	int isWriteBusy()
	{	// Linked to stopWrite/startWrite
		return XAxiVdma_IsBusy(&drv_inst_, XAXIVDMA_WRITE);
	}
	int isReadBusy()
	{	// Linked to stopRead/startRead
		return XAxiVdma_IsBusy(&drv_inst_, XAXIVDMA_READ);
	}
	void voidWriteRegister()
	{	// 
		XAxiVdma_DmaRegisterDump(&drv_inst_, XAXIVDMA_WRITE);
	}
	void voidReadRegister()
	{	// 
		XAxiVdma_DmaRegisterDump(&drv_inst_, XAXIVDMA_READ);
	}
	uint32_t getWriteStatus()
	{	// 
		return XAxiVdma_GetStatus(&drv_inst_, XAXIVDMA_WRITE);
	}
	uint32_t getReadStatus()
	{	// 
		return XAxiVdma_GetStatus(&drv_inst_, XAXIVDMA_READ);
	}
